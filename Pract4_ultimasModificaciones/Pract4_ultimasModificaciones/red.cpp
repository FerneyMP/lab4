...
void red::modificar_Enrutador(char clave)
{
    ...
    bool modo;
    char captura = '\0';
    map<char, enrutador>::iterator itera;
    enrutador enruta;
    if(redes.find(clave)!=redes.end()){
        ...
        //CORREGIR ------------------------------------
        if(redes.find(modificar)==redes.end()){ //CORREGIR
...
void red::origen_destino()
{
    ...
    if(redes.find(origen)!=redes.end()){
        cout << "Ingrese el destino: "; cin >> destino; //AGREGAR VALIDACION

        if(redes.find(destino)!=redes.end()){
            redes[origen].mirar_enrutadores_contiguos(origen,destino);}
        if(redes.find(destino)==redes.end()){
